...
export default function modalWindowDrawing(dataFromServer) {
  const urlPoster = "https://image.tmdb.org/t/p/w500";
  const swiperItems = document.querySelectorAll(".swiper-slide");

  for (const itemOfslider of swiperItems) {
    itemOfslider.addEventListener("click", () => showModalWindow(itemOfslider, dataFromServer.results));
  }
  ...
  function showModalWindow(itemOfslider, data) {
    ...
    (async () => {
      try {
        ...
        } else if (additionalData.homepage) {
